...
import com.example.presentation.viewmodels.SportsNewsViewModel
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.android.synthetic.main.fragment_base_news.*

@AndroidEntryPoint
class SportsNewsFragment : BaseNewsFragment() {

    private val sportsNewsViewModel: SportsNewsViewModel by viewModels()
    private lateinit var newsAdapter: NewsAdapter
    private val sportsNewsArticles: MutableList<NewsArticle> = mutableListOf()
    ...
    private val recyclerViewPaginator by lazy {
        object : RecyclerViewPaginator(recycler_view) {
            override fun loadMore(currentPage: Int) {
                sportsNewsViewModel.loadMoreSportsNews(currentPage)
            }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            sportsNewsViewModel.getSportsNews(page = 1, country = Country.US)
        }
    ...
    private fun setupRecyclerView() {
        newsAdapter = NewsAdapter(sportsNewsArticles, requireContext())
    ...
    private fun observeViewModel() {
        sportsNewsViewModel.sportsNewsLiveData.observe(owner) { viewState ->
            when(viewState) {
                is ViewState.Loading -> {
                    if (sportsNewsViewModel.pageNumber > 1) {
    ...
    companion object {
        @JvmStatic
        fun newInstance() = SportsNewsFragment()
    }
}
